...
# This script installs the agent-bin as a Windows Service.
# Must be run as Administrator.

$ServiceName = "AgentMonitor"
$BinaryPath = "C:\ProgramData\agente-monitoramento\agent-bin.exe"
$ConfigDir = "C:\ProgramData\agente-monitoramento"
$SourceBinary = Join-Path $PSScriptRoot "target\debug\agent-bin.exe"
...
if (!(Test-Path $SourceBinary)) {
    Write-Error "Source binary not found at $SourceBinary - please run 'cargo build -p agent-bin' first."
    exit 1
}
...
if ($serviceExists) {
    ...
    $serviceParams = @{
        ...
        DisplayName = "Agent Monitoring Service"
        StartupType = "Manual"
        Description = "Collects system snapshots and sends them to the monitoring server."
    }
    try {
